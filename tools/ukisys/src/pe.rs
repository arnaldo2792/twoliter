...
//!
//! The UKI section names this module truncates (`.osrel`, `.cmdline`,
//! `.uname`, `.linux`) are defined by the UAPI Group's Unified Kernel
...
pub enum PeError {
    ...
    #[snafu(display("optional header magic 0x{magic:04x} is not PE32+ (0x020b)"))]
    NotPe32Plus { magic: u16 },

    #[snafu(display(
        "file too short while reading {context}: needed at least {needed} bytes, have {have}"
    ))]
    TruncatedFile {
        needed: usize,
        have: usize,
...
/// within the file (so callers can locate/patch the raw table entry).
#[derive(Debug, Clone)]
pub struct Section {
    pub name: String,
...
/// A parsed view over an in-memory PE32+ image. Holds the raw bytes plus
/// the header offsets needed to read or patch fields in place.
pub struct PeImage {
    pub bytes: Vec<u8>,
    pub pe_offset: usize,   // offset of "PE\0\0"
    pub coff_offset: usize, // == pe_offset + 4
    pub opt_offset: usize,  // == pe_offset + 24
...
// ---- Fixed byte offsets, named per the spec in the module doc comment ----
const OFF_E_LFANEW: usize = 0x3C;
const COFF_NUM_SECTIONS: usize = 2; // relative to coff_offset
const COFF_SIZE_OPT_HDR: usize = 16; // relative to coff_offset
const OPT_MAGIC: usize = 0; // relative to opt_offset
...
/// Writes a little-endian u32 into `buf` at `offset` in place.
...
impl PeImage {
    ...
    pub fn parse(bytes: Vec<u8>) -> Result<Self, PeError> {
        ...
        for i in 0..num_sections as usize {
            ...
            if entry_offset + SECTION_ENTRY_SIZE > bytes.len() {
                return Err(PeError::TruncatedFile {
                    needed: entry_offset + SECTION_ENTRY_SIZE,
                    have: bytes.len(),
    ...
    /// Reads the Security Directory (Data Directory entry 4)
    ...
    fn set_security_directory(&mut self, rva: u32, size: u32) {
        let dir_offset = self.opt_offset + OPT_DATA_DIRECTORY + SECURITY_DIRECTORY_INDEX * 8;
        write_u32(&mut self.bytes, dir_offset, rva);
        write_u32(&mut self.bytes, dir_offset + 4, size);
    }
    ...
    fn set_number_of_sections(&mut self, value: u16) {
        write_u16(&mut self.bytes, self.coff_offset + COFF_NUM_SECTIONS, value);
    }
    ...
    fn set_size_of_initialized_data(&mut self, value: u32) {
        write_u32(
            &mut self.bytes,
            self.opt_offset + OPT_SIZE_OF_INIT_DATA,
            value,
    ...
    fn set_size_of_image(&mut self, value: u32) {
        write_u32(&mut self.bytes, self.opt_offset + OPT_SIZE_OF_IMAGE, value);
    }
    ...
    fn set_checksum(&mut self, value: u32) {
        write_u32(&mut self.bytes, self.opt_offset + OPT_CHECKSUM, value);
    }
    ...
    /// splice (no kept section's file offset needs to move).
    ///
    /// `trailing_names` must exactly match, in order, the last
    /// `trailing_names.len()` entries of the section table; otherwise this
    /// returns `UnexpectedTrailingSections` rather than silently truncating
    /// the wrong data.
    ///
    ...
    pub fn derive_stub_by_truncating_trailing_sections(
        ...
        // no longer claim these entries exist.
        ...
        self.set_size_of_image(new_size_of_image as u32);
...
mod tests {
    ...
    /// a real UKI on disk. Not a faithful PE (no real code/data), just
    /// enough structure for the header/section-table logic under test.
    ///
    ...
    fn build_synthetic_pe(section_specs: &[(&str, u32, u32, u32, u32)]) -> Vec<u8> {
        ...
        buf[0] = b'M';
        buf[1] = b'Z';
        write_u32(&mut buf, OFF_E_LFANEW, e_lfanew);
        ...
        write_u16(&mut buf, coff_offset + COFF_SIZE_OPT_HDR, opt_header_size);
        write_u16(&mut buf, opt_offset + OPT_MAGIC, PE32_PLUS_MAGIC);
        write_u32(&mut buf, opt_offset + OPT_SECTION_ALIGNMENT, 0x1000);
        write_u32(&mut buf, opt_offset + OPT_SIZE_OF_INIT_DATA, 0);
        write_u32(&mut buf, opt_offset + OPT_CHECKSUM, 0xdead_beef);
        ...
        for (i, (name, vsize, vaddr, rawsize, ptr)) in section_specs.iter().enumerate() {
            ...
            write_u32(&mut buf, off + 8, *vsize);
            write_u32(&mut buf, off + 12, *vaddr);
            write_u32(&mut buf, off + 16, *rawsize);
            write_u32(&mut buf, off + 20, *ptr);
    ...
    fn rejects_non_pe32_plus() {
        ...
        write_u16(&mut bytes, opt_offset + OPT_MAGIC, 0x010B);
    ...
    fn security_directory_roundtrip() {
        ...
        img.set_security_directory(cert_offset, 0x400);
